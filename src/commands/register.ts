...
import { Routes, APIApplicationCommandOptionChoice } from 'discord-api-types/v10';
    ...
    new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                ...
                .setDescription('Add or remove a channel where the bot is allowed to speak')
        ...
        .addSubcommand(subcommand =>
            subcommand
                ...
                .setDescription('Set how responsive the bot is (0.1=less, 1.0=normal, 2.0=more)')
                ...
                .addStringOption(option => {
                    ...
                    try {
                        ...
                        presetPersonas.forEach((persona, id) => {
                            ...
                            const choiceName = persona.name.length > 90 ? `${persona.name.substring(0, 90)}... (${id})` : `${persona.name} (${id})`;
                        ...
                        if (choices.length > 0) {
                            ...
                            loggerService.logger.warn("No preset personas found to add as choices for /config set_persona command.");
                         ...
                         loggerService.logger.error("Failed to load preset personas for command choices:", error);
                         // Add a fallback choice indicating an error?
                         option.addChoices({ name: "Error loading personas", value: "_error" });
                    }
...
(async () => {
    try {
        ...
        loggerService.logger.info('Successfully reloaded application (/) commands.');
    } catch (error) {

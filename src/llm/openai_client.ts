...
export async function callChatCompletionApi(
    ...
    if (!config) {
        loggerService.logger.error("Configuration is not loaded in openai_client.ts. Cannot call API.");
        return null;
    }
    ...
    try {
        loggerService.logger.debug(`Calling ${clientIdentifier} Chat Completion API: ${model} at ${client.baseURL} with ${messages.length} messages.`);
        ...
        if (!content) {
            throw new Error(`No content in response from LLM API at ${client.baseURL}`);
        }
        ...
        if (error instanceof OpenAI.APIError) {
            ...
            const errDetails = error.error ? JSON.stringify(error.error) : 'No details';
             loggerService.logger.error(
                `OpenAI API Error calling ${model} via ${clientIdentifier} client: ${status} ${errName} ${errMessage}`, errDetails
            );
             ...
             loggerService.logger.error(
                `Unknown Error calling LLM API ${model} via ${clientIdentifier} client:`, error
            );
        }
